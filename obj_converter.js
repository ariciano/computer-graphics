...
fs.readFile( __dirname + '/Homer_Donut.obj', function (err, data) {
	if (err) {
		throw err; 
	}
...
function splitObjects(data){
	...
	for(var max = data.length, i = 0; i < max; i ++){
		if(data[i].substring(0,2) == "v "){
			...
			wireframe.vertices[vCount] = coords.slice(2,5);
	...
	fs.writeFile('homer_donut.json', JSON.stringify(wireframe), function (err) {
		if (err) throw err;
